...
const styles = {
  ...
  navContainer: "container mx-auto px-4 md:px-12 lg:px-24 max-w-7xl flex justify-between items-center",
  logo: "text-2xl font-serif font-bold text-stone-800 cursor-pointer",
...
export default function App() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // В реальном проекте здесь будет путь к вашему JSON файлу
        // Для примера используем заглушку, если файла нет, чтобы код работал
        ...
        setData({
          ...
          services: [
            { id: 1, title: "Манікюр + Покриття", description: "Комбінований манікюр з ідеальним вирівнюванням.", price: "600 грн" },
  ...
  const scrollToSection = (e, href) => {
    ...
    // Закрываем мобильное меню сразу при клике
    setIsMobileMenuOpen(false);

    const element = document.querySelector(href);
    if (element) {
      // Вычисляем позицию с учетом фиксированного хедера
      // На мобильном хедер может быть выше или ниже, берем с запасом
    ...
    <div className={styles.app}>
      ...
      <nav className={isScrolled ? styles.nav : styles.navTransparent}>
        <div className={styles.navContainer}>
          ...
          {/* Mobile Menu Toggle */}
          <button 
            className="md:hidden text-stone-800 p-2 z-50 relative" 
